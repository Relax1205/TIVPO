...
## Ошибка 2: Кнопка C не всегда полностью очищает дисплей

1. **Идентификатор**  
   TC-002

2. **Название**  
   Проверка полной очистки выражения кнопкой C

3. **Описание**  
   Убедиться, что при нажатии кнопки `C` выражение полностью сбрасывается и не восстанавливается позже.

4. **Предварительные условия**  
   а) калькулятор запущен;  
   б) введено любое выражение, например, `5 + 3`.

5. **Шаги выполнения**  
   а) ввести выражение, например, `5 + 3`;  
   б) нажать кнопку `C`.

6. **Ожидаемый результат**  
   Дисплей отображает `0`, выражение полностью очищено, никакие изменения не происходят после этого.

7. **Фактический результат**  
   Через 500 мс после очистки дисплей возвращается к предыдущему значению (`5 + 3`), если выражение не было перезаписано.
...
## Ошибка 3: Операция умножения выполняет сложение
...
## Ошибка 4: Нарушение приоритета операций (вычисление слева направо)
...
## Ошибка 5: Первое нажатие кнопки = игнорируется
...
## Ошибка 6: Интерфейс ломается при малом разрешении экрана
...
## Ошибка 7: Отсутствие валидации ввода нескольких точек

1. **Идентификатор**  
   TC-007

2. **Название**  
   Проверка ввода нескольких десятичных точек в числе

3. **Описание**  
   Убедиться, что калькулятор не позволяет вводить более одной точки в одном числе.
...
6. **Ожидаемый результат**  
   Ввод второй точки блокируется или после нажатия `=` отображается `"Error"`.

7. **Фактический результат**  
   Калькулятор принимает `1.2.3` как валидный ввод, что приводит к некорректным вычислениям.
...
## Ошибка 8: Невозможность продолжения вычислений после результата

1. **Идентификатор**  
   TC-008

2. **Название**  
   Проверка возможности продолжения вычислений после получения результата

3. **Описание**  
   Убедиться, что после получения результата можно продолжить вычисления, используя его как начальное значение.

4. **Предварительные условия**  
   а) калькулятор запущен;  
   б) выполнено вычисление, например, `5 + 3 =`.

5. **Шаги выполнения**  
   а) ввести `5 + 3 =` → результат `8`;  
   б) нажать `+ 2 =`.

6. **Ожидаемый результат**  
   Результат: `10` (продолжение вычислений: `8 + 2`).

7. **Фактический результат**  
   Выражение не обновляется корректно, результат может быть неверным или операция не выполняется.
