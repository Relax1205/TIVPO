...
def bubble_sort_mutant3(arr):
    ...
    n = len(sorted_arr)
    for i in range(n):
        for j in range(0, n - i + 1):  # ОШИБКА: +1 вместо -1
            if j + 1 < n:  # Защита только от IndexError, но логика сломана
                if sorted_arr[j] > sorted_arr[j + 1]:
                    sorted_arr[j], sorted_arr[j + 1] = sorted_arr[j + 1], sorted_arr[j]
    return sorted_arr
...
def run_test_with_mutant(mutant_func):
    ...
    if 'test_array_operations' in sys.modules:
        importlib.reload(sys.modules['test_array_operations'])
