# mutation_test_password_generator.py
...
def check_password_strength_mutant3(password):
    """Не проверяет заглавные буквы"""
    if (password_generator.has_lowercase(password) and
        password_generator.has_digits(password) and
        any(c in "!@#$%&*" for c in password)):
        return "strong"
    return "weak"
...
mutants = [
    ...
    ("Mutant 3: No uppercase check", check_password_strength_mutant3, "check_password_strength"),
    ("Mutant 4: Always strong", check_password_strength_mutant4, "check_password_strength"),
...
if __name__ == "__main__":
    ...
    for name, mutant_func, func_name in mutants:
        ...
        # Запускаем тесты
        all_tests_passed = run_all_tests('test_password_generator')
